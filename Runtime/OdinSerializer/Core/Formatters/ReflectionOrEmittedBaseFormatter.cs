...
namespace OdinSerializer
{
    public abstract class ReflectionOrEmittedBaseFormatter<T> : ReflectionFormatter<T>
    {
        ...
        {
            var formatter = FormatterEmitter.GetEmittedFormatter(typeof(T), reader.Context.Config.SerializationPolicy, false) as FormatterEmitter.RuntimeEmittedFormatter<T>;
        ...
        {
            var formatter = FormatterEmitter.GetEmittedFormatter(typeof(T), writer.Context.Config.SerializationPolicy, false) as FormatterEmitter.RuntimeEmittedFormatter<T>;

            if (formatter == null)
            {
                base.SerializeImplementation(ref value, writer);
                return;
            }
