...
namespace NKGGameFramework.Core
{
    ...
    public sealed class EventBus : IEventBus
    {
        ...
        private readonly Queue<IQueuedEvent> _queuedEvents = new();
        private readonly Dictionary<Type, IGameEventArgsPool> _eventArgsPools = new();
        ...
        private sealed class QueuedEvent<TEvent> : IQueuedEvent
        {
            private readonly TEvent _evt;
            private readonly bool _releaseAfterDispatch;

            public QueuedEvent(TEvent evt, bool releaseAfterDispatch)
            {
                _evt = evt;
                _releaseAfterDispatch = releaseAfterDispatch;
            }
            ...
            public void Dispatch(EventBus eventBus)
            {
                try
                {
                    eventBus.FireNow(_evt);
                }
                finally
                {
                    if (_releaseAfterDispatch && _evt is GameEventArgs eventArgs)
                    {
