...
namespace NKGGameFramework.Nodes
{
    ...
    public sealed class NodeGraph
    {
        ...
        public Dictionary<long, Node> Nodes { get; } = new();

        public Dictionary<long, NodePortLine> PortLines { get; } = new();
        ...
        {
            NkgThrow.IfNull(first);
            NkgThrow.IfNull(second);
    ...
    public sealed class NodePort : IPoolItem
    {
        ...
        private readonly List<NodePortLine> _connectionLines = new();
    ...
    public sealed class NodePortLine : IPoolItem
    {
        ...
        public List<NodePoint> ReroutePoints { get; init; } = new();
    ...
    public readonly struct NodePoint : IEquatable<NodePoint>
    {
        ...
        public bool Equals(NodePoint other) => EqualityComparer<float>.Default.Equals(X, other.X) && EqualityComparer<float>.Default.Equals(Y, other.Y);

        public override int GetHashCode() => HashCode.Combine(X, Y);

        public override bool Equals(object? obj) => obj is NodePoint other && Equals(other);

        public static bool operator ==(NodePoint left, NodePoint right) => left.Equals(right);
        public static bool operator !=(NodePoint left, NodePoint right) => !left.Equals(right);
    ...
    {
        private static readonly Dictionary<Type, IReadOnlyList<NodePortTemplate>> PortTemplates = new();
    ...
    internal readonly struct NodePortTemplate : IEquatable<NodePortTemplate>
    {
        public readonly string Name;
        public readonly string FieldName;
        public readonly Type ValueType;
        public readonly NodePortDirection Direction;
        public readonly NodePortPosition Position;
        public readonly NodeConnectionType ConnectionType;
        public readonly NodeTypeConstraint TypeConstraint;
        public readonly bool ShowName;

        public NodePortTemplate(string Name, string FieldName, Type ValueType, NodePortDirection Direction, NodePortPosition Position, NodeConnectionType ConnectionType, NodeTypeConstraint TypeConstraint, bool ShowName)
        {
            this.Name = Name;
        ...
        public bool Equals(NodePortTemplate other) => EqualityComparer<string>.Default.Equals(Name, other.Name) && EqualityComparer<string>.Default.Equals(FieldName, other.FieldName) && EqualityComparer<Type>.Default.Equals(ValueType, other.ValueType) && EqualityComparer<NodePortDirection>.Default.Equals(Direction, other.Direction) && EqualityComparer<NodePortPosition>.Default.Equals(Position, other.Position) && EqualityComparer<NodeConnectionType>.Default.Equals(ConnectionType, other.ConnectionType) && EqualityComparer<NodeTypeConstraint>.Default.Equals(TypeConstraint, other.TypeConstraint) && EqualityComparer<bool>.Default.Equals(ShowName, other.ShowName);

        public override int GetHashCode() => HashCode.Combine(Name, FieldName, ValueType, Direction, Position, ConnectionType, TypeConstraint, ShowName);

        public override bool Equals(object? obj) => obj is NodePortTemplate other && Equals(other);

        public static bool operator ==(NodePortTemplate left, NodePortTemplate right) => left.Equals(right);
        public static bool operator !=(NodePortTemplate left, NodePortTemplate right) => !left.Equals(right);

        public void Deconstruct(out string name, out string fieldName, out Type valueType, out NodePortDirection direction, out NodePortPosition position, out NodeConnectionType connectionType, out NodeTypeConstraint typeConstraint, out bool showName)
        {
            name = Name; fieldName = FieldName; valueType = ValueType; direction = Direction; position = Position; connectionType = ConnectionType; typeConstraint = TypeConstraint; showName = ShowName;
        }
    }
}
