...
namespace NKGGameFramework.Nodes
{
    ...
    public sealed class NodeGraphIndex
    {
        ...
        {
            return _outgoingLinks.TryGetValue(nodeId, out var links) ? links : Array.Empty<NodeLinkDefinition>();
        }

        public IReadOnlyList<NodeLinkDefinition> GetIncomingLinks(string nodeId)
        {
            return _incomingLinks.TryGetValue(nodeId, out var links) ? links : Array.Empty<NodeLinkDefinition>();
        }
